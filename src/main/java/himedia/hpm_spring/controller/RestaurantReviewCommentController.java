...
public class RestaurantReviewCommentController {
	...
	public ResponseEntity<List<RestaurantReviewCommentVo>> retrieveAllComments(@PathVariable Long mReviewId) {
	    List<RestaurantReviewCommentVo> comments = rCommentService.retrieveAllComments(mReviewId);
	...
	public ResponseEntity<RestaurantReviewCommentVo> createComment(@PathVariable Long rReviewId, @RequestBody RestaurantReviewCommentVo comment) {
	    if (comment.getUsersId() == null || comment.getUsersId() <= 0) {
	        return ResponseEntity.badRequest().body(null);
	    }

	    // 커뮤니티 ID 설정
	    comment.setRestaurantsId(rReviewId);
	...
	// POST : /api/restaurant-reviews/{rReviewId}/comments/{commentId}/replies -> 대댓글 생성
	@PostMapping("/{rReviewId}/comments/{commentId}/replies")
	public ResponseEntity<RestaurantReviewCommentVo> createReply(
	        @PathVariable Long rReviewId,
	        @PathVariable Long commentId,
	        @RequestBody RestaurantReviewCommentVo reply) {
	    
	    reply.setRestaurantsId(rReviewId);
	...
	public ResponseEntity<RestaurantReviewCommentVo> updateReply(
